ui<-fluidPage(
  ...
  navbarPage("PhylteR", id="PhylteR",selected = "Welcome",
      ...
      column(10, offset = 1,
        helpText(
          HTML("<h2 align='center'>Welcome on the PhylteR application online!</h2>
            ...
            <p align='center'> - <u>k</u> is a threshold of the outlier detection. The bigger it is, the more stringent the detection is. </p>
            <hr style='height: 2px; color: #FF4000; background-color: #FF4000; width: 50%; border: none;'>
    ...
    tabPanel("visualize distances between species",
      helpText("Grey circles are the mean of distance between species for a particular specie. Red circles are genes. For a given gene, the distance between the specie of interest and an other one is given by the shape of the red line: an acute angle mean an important distance between two species"),
      plotOutput(outputId = "plot6", height = "1500px")
    ),
    tabPanel("visualize distances between genes",
      helpText("For a given gene, each red line correspond to a specie and each point on the red line correspond to the distance between this specie and the other species. Grey circle is the mean distance between species for this given gene."),
