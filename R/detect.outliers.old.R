...
detect.outliers<-function(mat2WR, k=3, thres=0.3, test.island=TRUE, keep.species=TRUE) {
  ...
  Cells<-rbind(CompOutl$cells, CellOutl$cells)
  return(rbind(CompOutl$cells, CellOutl$cells))
}
...
detect.complete.outliers <- function(mat2WR, k = 3, thres = 0.3, keep.species=TRUE) {
  ...
  #conversion of these outliers as cells.
  ...
  if (keep.species) RES$cells <- cells.outgn
  else {
