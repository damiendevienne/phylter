...
You may want to run **phylter** and to subsequently remove the identified outliers from both the gene trees and the sequences files. For this to be performed, the sequence files must contain the same name as the corresponding tree, minus the extension if any. For example, a sequence file named `ENSG00000274286_ADRA2B_final_align_NT.aln` will be matched automatically to a tree file named `ENSG00000274286_ADRA2B.treefile`: `phylter.py` will identoify the gene ID as being **ENSG00000274286_ADRA2B**.
...
- `-g TRUE` specifies that a full report (in pdf) should be produced.
...
4. See all **phylter** options. 
phylter.py allows specifying all the options available in the R package. To see this list of options, simply type:
```sh
...
PhylteR is also available as a [singularity](https://docs.sylabs.io/guides/3.0/user-guide/index.html) container : (https://cloud.sylabs.io/library/theo.treecou/tool/phylter_singularity):
...
2. Running PhylteR on the carnivora example dataset:

```sh
singularity exec PhylteR.sif phylter.py -j Carnivora_singularity -t /usr/container-data/trees
```

Please, refer to the description of the docker container to see how to use the options available with the `phylter.py` function.
