...
the output file `phylter.out` will be used for performing the pruning and/or the sequence filtering (see below).
 ...
 2. Prune trees with `prune_tree_outliers.R`: 
 `sudo docker run -v $PWD:$PWD -w $PWD treecoutheo/ BLABLABLA`
 
 3. Filter alignments with `remove_sequence_outliers.py`: 
`sudo docker run -v $PWD:$PWD -w $PWD treecoutheo/ BLABLABALABJDLHUIDFHDUI`
