ui<-fluidPage(
  ...
  tags$head(tags$script("
        window.onload = function() {
            ...
            $('#PhylteR a:contains(\"visualize correlation between genes\")').parent().addClass('disabled');
        };
  ...
  wellPanel(
    fluidRow(
      column(6,
        ...
        bsPopover("q1", "help", "Enter a list of gene trees in Newick format. This file should not exceeds 4Mo, if it is the case, please use the R functions.", placement = "bottom", trigger = "click", options = NULL),
      ...
      column(6,
        fluidRow(
          ...
          column(6,
            radioButtons(inputId="impMet",label = "Imputation of missing data", choiceNames = c("IPCA (accurate but slower)","MEAN (less accurate but faster)"), choiceValues = c("IPCA","MEAN"), selected = "MEAN")
          )
        ),
        sliderInput(inputId="k", label = "select k", value = 3, min=1, max=6, round=FALSE, step=0.1),
        #uiOutput(outputId = "outbvalue"),
        useShinyjs(),
        disabled(
          sliderInput("bvalue", label = "Collapse nodes having bvalues lower than : ", value = 0, min = 0, max = 100, step = 5)
        ),
        textOutput(outputId = "DataSetInfo"),
        textOutput(outputId = "BvalueInfo")
      )
    ),
    fluidRow(
      htmlOutput(outputId = "DataSetProblem")
      #downloadButton("downloadOut", label = "Download the outliers list")
    )
  ),
    ...
    tabPanel("Welcome",
      fixedRow(
        column(10, offset = 1,
          helpText(
            HTML("
<h2 align='center'>Welcome to the PhylteR application!</h2>
...
Stéphane Dray - <a href='mailto:stephane.dray@univ-lyon1.fr'>stephane.dray@univ-lyon1.fr</a><br>
    ...
    tabPanel("visualize some trees",
      column(3, offset = 0,
        ...
        conditionalPanel(condition = "input.Outlier == 'on'",
          helpText(
            HTML("
             <br>
             <p align='left'>
             <img src='legendeTree.png' width='80%' height='80%'>
             </p>
            ")
          )
        )
      ...
      mainPanel(
        plotOutput(outputId = "plot1", height = "2000px")
      )
    ),
        ...
        conditionalPanel(condition = "input.Outlier == 'on'",
          helpText(
           HTML("
             <br>
             <p align='left'>
             <img src='legende.png' width='70%' height='70%'>
             </p>
            ")
          )
        )
    ...
    tabPanel("visualize correlation between genes",
      helpText("Heatmap of the correlation matrix between genes"),
      plotOutput(outputId = "plot8", height = "1000px", dblclick="plot8dbclick", brush = brushOpts(id = "plot8_brush",resetOnNew = TRUE))
    ),
    ...
    tabPanel("visualize distances between species",
      ...
      plotOutput(outputId = "plot6", height = "1400px")
    ),
    tabPanel("visualize distances between genes",
      ...
      plotOutput(outputId = "plot7", height = "1400px")
    )
  )
)
